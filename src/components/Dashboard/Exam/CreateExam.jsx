...
const CreateExam = () => {
  ...
  useEffect(() => {
    ...
    // Log the updated questions for debugging
  ...
  const handleAddQuestion = () => {
    ...
    if (questionText.trim() && clo.trim()) {
      const newQuestion = {
        ...
        questionType, // "objective" or "subjective"
        text: questionText,
        clo,
      };
  ...
  const handleUpdateQuestion = () => {
    if (currentQuestion.text.trim() && currentQuestion.clo.trim()) {
        ...
        <table className="min-w-full border border-gray-200 rounded-lg shadow">
          <thead>
            <tr className="bg-blue-100 text-blue-700 text-left">
              ...
              <th className="p-3 border border-gray-400 w-1/5">
                Question Type
              </th>
              <th className="p-3 border border-gray-400 w-1/5">CLO</th>
              <th className="p-3 border border-gray-400 text-center w-1/5">
                Actions
              </th>
            </tr>
          </thead>
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Render tables for Midterm and Final questions */}
      {renderTable(
        questions.filter((q) => q.examType === "midterm"),
        "midterm"
      )}
      {renderTable(
        questions.filter((q) => q.examType === "final"),
        "final"
      )}
      ...
      {isEditModalOpen && currentQuestion && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-[40%]">
            <h2 className="text-lg font-semibold mb-4">
              {examName ? decodeURIComponent(examName) : "Exam"} -{" "}
              {examType.charAt(0).toUpperCase() + examType.slice(1)} Questions
            </h2>
            <div className="mb-4">
              <label className="block text-blue-700 font-semibold mb-2">
